...
import { NavigationContainer, StackActionHelpers, DefaultTheme } from '@react-navigation/native';
...
const AuthenticatedApp = (props) => {
    ...
    const OnBoardingScreens = () => {
        ...
        const screenIDs = onBoardingSlugs?.map(slug => initialState.screens.find(item => item.slug === slug))?.map(screen => screen.id)
        ...
        return (
            <OnBoardingStack.Navigator
                ...
                initialRouteName={screenIDs[0]}>
                    {screenIDs
                    ?.map((item, index) => {
                        console.log('OnBoardingStack item', item);
                            return (
                    ...
                    ?.map((item, index) => {
                        if(item.submenuitems?.length > 0){
                            ...
                            item.submenuitems?.filter(
                                item => item?.visible === true).map(el => items.push(el))
                            items.map(item => {
                                return (        
                                    <MyStack.Screen 
                ...
                {initialState.screens
                    ...
                    .map((item, index) => {
                        return <Stack.Screen
                            ...
                            component={item.isLoadMore ? ScreenGenieLoadMore : ScreenGenie}
                            key={item.id}
                    ...
                    <SafeAreaProvider>
                        <onBoardingContext.Provider value={{
                            ...
                            {initialState.adminSections && <View style={{flex: 1, backgroundColor:'white'}} >
                                ...
                                <OnBoardingScreens />
                                {/* <View><Text style={{color:'red', fontSize: 100}} >some text</Text></View> */}
                            </View>}
