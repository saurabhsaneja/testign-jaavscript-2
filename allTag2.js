...
const ScreenGenie = ({ route, navigation, screen }) => {
  ...
  const getScreenData = useCallback(async (
    ...
    console.log('resolvedData2', resolvedData);
    ...
    const foundSearch = resolvedDataCopy.find(el => el?.slug === 'search')
    console.log('foundSearch', foundSearch);
    if(foundSearch){
      resolvedDataCopy = resolvedDataCopy.filter(el => el?.slug !== 'search')
      // console.log('found', found);
      if(foundSearch?.data){
        setIsTagDataReady(true)
        console.log('foundSearch2', JSON.stringify(foundSearch));
        setTagData(foundSearch?.data)
      }
    }
...
const fetchStory = async () => {
  ...
  const render = useCallback(({ item, index }) => {
    ...
    const data = route?.params?.tag?.slug ? tagData : foundSection ? foundSection.data : [];
