...
const AuthenticatedApp = (props) => {
    ...
    useEffect(() => {
        if (initialised) {
            console.log('lalaland initialised initialState.isLoading', initialState.isLoading);
            console.log('lalaland dataFetchList', dataFetchList);
            console.log('lalaland dataFetchList?.length !== 0', dataFetchList?.length !== 0);
            console.log('lalaland dataFetchList.every(item => item?.loading === false)', dataFetchList.every(item => item?.loading === false));

            console.log('weird condition', dataFetchList?.length !== 0 && initialState.isLoading && dataFetchList.filter(el => el !== null).every(item => item?.loading === false))
            if(dataFetchList?.length !== 0 && initialState.isLoading && dataFetchList.every(item => item?.loading === false)) {
                ...
                console.log('dataFetchList', dataFetchList);
