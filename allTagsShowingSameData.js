...
const ScreenGenie = ({ route, navigation, screen }) => {
  ...
  const isDataReady = () => {
    ...
    for(const section of filteredSections){
      ...
      if(section?.slug === 'search'){
        if(found?.data){
          const foundSlug = found?.data?.find(el => el?.slug === route?.params?.tag?.slug)
          if(!foundSlug?.data){
            dataReady = false
            break
          }  
  ...
  const getScreenData = useCallback(async (
    ...
    if(route?.params?.tag?.slug){
      resolvedDataCopy = resolvedDataCopy?.map(el => {
          if(el?.slug === 'search'){
            const found = dataFetchList?.find(el => el?.slug === 'search')
            if(found?.data && Array.isArray(found?.data) && found?.data?.length > 0){
              if(el?.data){
                const newData = [...found?.data, {slug: route?.params?.tag?.slug, data: el?.data}]
                return {
                  ...el,
                  data: {slug: route?.params?.tag?.slug, data: newData}}
              }else {
                const newData = [...found?.data, {slug: route?.params?.tag?.slug, error: {}}]
                return {
                  ...el,
                  data: {slug: route?.params?.tag?.slug, data: newData}}  
              }
              ...
              if(el?.data){
                ...
                return {
                  ...el,
                  data: [{slug: route?.params?.tag?.slug, error: {}}]}  
              }
            }
          }else {
            return el
  ...
  const fetchDataAsync = async (
    ...
    try {
      ...
      if (data?.length && slugExists) {
        ...
        if(slug === 'search'){
          console.log('slug === search', slug, data);
        }
        return {slug, data}
      } else {
...
const fetchTagResults = async () => {
    console.log('topicStoriesQuery', route?.params?.tag?.slug);
    let tagResults = {}
    try {
        const storiesByTopicQuery = PatrikaQueries.topicStoriesQuery
        const variables = {
            slug: route?.params?.tag?.slug
        }
        tagResults = await client.query({
            query: storiesByTopicQuery,
            variables: variables,
        });
        console.log('topicStoriesQuery data', JSON.stringify(tagResults?.data?.storiesByTopic));
...
const fetchStory = async () => {
  ...
  const render = useCallback(({ item, index }) => {
    ...
    let foundSection = {}
    if(item.slug === 'search'){
      const foundSearchSection = dataFetchList.find(
        (section) => section.slug === item.slug && section.loading === false
      );
      foundSection = foundSearchSection?.data?.find(el => el?.slug === route?.params?.tag?.slug)
    }else {
      foundSection = dataFetchList.find(
        (section) => section.slug === item.slug && section.loading === false
      );
    }
