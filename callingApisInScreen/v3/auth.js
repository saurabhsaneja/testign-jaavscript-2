...
const AuthenticatedApp = (props) => {
    ...
    const fetchDataAsync = useCallback(async (name) => {
        let data = []
        let slugExists = false

        const missingStoryCollectionSlugs = []
        const missingNonStoryCollectionSlugs = []
        const allStoryCollectionSlugs = initialState.sections.filter(section => section.sectionType === SectionType.STORYCOLLECTION)?.map(el => el?.slug)
        const filteredSections = getFilteredSections(name)
        ...
        if(missingStoryCollectionSlugs?.length > 0){
            data = await fetchStoriesByCategories(missingStoryCollectionSlugs?.map(el => ({slug: el})));
            console.log('fetchStoriesByCategories', data);
            slugExists = true
            const slug = 'storiesByCategory'
        ...
        if(missingNonStoryCollectionSlugs?.length > 0){
            missingNonStoryCollectionSlugs?.map(slug => slug)
            switch (slug) {
                case 'tag-list':
    ...
    const fetchStoriesByCategories = useCallback(async (customSections) => {
        let storiesSlugMap = {}
        try {
    ...
    const getFilteredSections = (name) => {
        let filteredSections = initialState.sections.filter(
            (id) => name === id.screenID && id.visible === true
        )

        return filteredSections
    }

    const getMissingSlugs = (name) => {
        const missingStoryCollectionSlugs = []
        const missingNonStoryCollectionSlugs = []
        const allStoryCollectionSlugs = initialState.sections.filter(section => section.sectionType === SectionType.STORYCOLLECTION)?.map(el => el?.slug)
        const filteredSections = getFilteredSections(name)
        ...
        return {missingStoryCollectionSlugs, missingNonStoryCollectionSlugs}  
    }
    const getNonStoryCollectionData = async (name) => {
        const missingNonStoryCollectionSlugs = []
        const allStoryCollectionSlugs = initialState.sections.filter(section => section.sectionType === SectionType.STORYCOLLECTION)?.map(el => el?.slug)
        const filteredSections = getFilteredSections(name)
        filteredSections?.map(item => {
        ...
        if(missingNonStoryCollectionSlugs?.length > 0){
            missingNonStoryCollectionSlugs?.map(async el => {
              const res = await fetchDataAsync(el)
              console.log('res', res);  
              nonStoryCollectionData.push(res)
            })
        }
        console.log('nonStoryCollectionData2', nonStoryCollectionData);
        return nonStoryCollectionData
    }
    const getStoryCollectionData = async (name) => {
        return []
        const missingStoryCollectionSlugs = []
        const allStoryCollectionSlugs = initialState.sections.filter(section => section.sectionType === SectionType.STORYCOLLECTION)?.map(el => el?.slug)
        const filteredSections = getFilteredSections(name)
                ...
                .map((item, index) =>
                    <Tab.Screen
                        ...
                        initialParams={{ ...initialParams, 
                            ...
                            getFilteredSections,
                            getMissingSlugs,
                            getNonStoryCollectionData,
                            getStoryCollectionData 
                        }}
                    />
                )
            }
