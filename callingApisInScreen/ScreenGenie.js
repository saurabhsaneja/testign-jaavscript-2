...
const ScreenGenie = ({ route, navigation, screen }) => {
  ...
  const { screens, sections, fetchDataAsync } = useContext(initContext);
    ...
    if(currentRoute.name !== 'video-story-screen'){
      async function filterSections() {
        try {
          ...
          console.log('screengenie dataFetchList', dataFetchList);
          // setFilteredSections(filteredSections?.map(item => dataFetchList.find(
          filteredSections?.map(item => dataFetchList.find(
            async (section) => {
              console.log('screengenie section', section);
              console.log('screengenie item', item);
              if(section.slug === item.slug && section?.loading === false){
                if(section.data){
                  return {...item, data: section.data, loading: false}
                }else {
                  return {...item, error: {}, loading: false}
                }
              }
              else {
                const res = await fetchDataAsync(item?.slug)
                console.log('fetching data', item?.slug, res);
                if(res?.data?.length && res?.slugExists){
                  return {...item, data: res?.data, loading: false}
                }else {
                  data.push({...item, error: {}, loading: false}) 
                }
              }
            }
          ))
  ...
  return (
    ...
    >
      ...
      :
      filteredSections.length && filteredSections?.map(el => el.data) ? (
        <FlatList
