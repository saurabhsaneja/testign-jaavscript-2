...
const AuthenticatedApp = (props) => {
    ...
    const getOnboardingScreenIDs = async () => {
        ...
        return onBoardingSlugs?.map(slug => screens.find(item => item.slug === slug))?.map(screen => screen.id)
    } 
    const OnBoardingScreens = async () => {
        const screenIDs = await getOnboardingScreenIDs()
