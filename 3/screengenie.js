...
const ScreenGenie = ({ route, navigation, screen }) => {
  ...
  const { dataFetchList, batchDataUpdate, batchInitialise, fetchDataSuccess, fetchDataFailure } = useDataStore();
  ...
  useEffect(() => {
    console.log('dataFetchList changed', dataFetchList);
  }, [dataFetchList])
  useEffect(() => {
  ...
  const getScreenData = useCallback(async (
    ...
    if(dataFetchList?.length === 0){
      if (sections.length !== 0) {
          sections.filter(section =>
              ...
              if(section) slugFest.push({slug: section.slug, loading: true})
    ...
    console.log('resolvedDataCopy1', resolvedDataCopy);
    ...
    if(found){
      ...
      if(found?.data){
        ...
        resolvedDataCopy.push(...missingStoryCollectionSlugs?.map(el => ({slug: el, error: {}})))
      }
      resolvedDataCopy = resolvedDataCopy.filter(el => el?.slug !== 'storiesByCategory')
    }
    console.log('resolvedDataCopy2', resolvedDataCopy);
    if(resolvedDataCopy?.length > 0){
      if(dataFetchList?.length === 0){
        slugFest = slugFest?.map(el => {
          const found = resolvedDataCopy?.find(el2 => el2?.slug === el?.slug)
          console.log('found', found);
          if(found){
            return {
              ...found,
        ...
        if(!isInitial){
          console.log('batchInitialise');
          batchInitialise(slugFest)
          setIsInitial(true)
        }
      }else {
        console.log('batchDataUpdate');
  ...
  const fetchDataAsync = async (
    ...
    try {
      ...
      if(slug === 'storiesByCategory'){
        console.log('for storiesByCategory', data);
      }
  ...
  const fetchStoriesByCategories = async (customSections) => {
    ...
    try {
        ...
        console.log("Error for storiesByCategory", error)
        return []
    }
}
...
const fetchStory = async () => {
  ...
  const render = useCallback(({ item, index }) => {
    console.log('isDataReady() render', isDataReady(), dataFetchList);
    ...
    console.log('render dataFetchList', dataFetchList);
